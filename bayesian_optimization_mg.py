...
U_M = 0.152
U_D = 5.95e-3
K_N = 30.0e-3 * 1000  # Converted from g/L to mg/L
Y_NX = 0.305
K_M = (0.350e-3 * 2) * 1000 # SCALED K_M by 1000 for consistency to produce Lutein in mg/L rate
K_D = 3.71 * 0.05 / 90 # Rate constant, units compatible with mg/L already
K_NL = 10.0e-3 * 1000 # Converted from g/L to mg/L
K_S = 142.8
K_I = 214.2
K_SL = 320.6
K_IL = 480.9
TAU = 0.120 # Absorption coefficient. Assumed to be compatible with concentration in mg/L and path length in cm.
KA = 0.0
...
# This is a ratio: g Lu / 1 g Biomass. Numerically, it's also mg Lu / 1 mg Biomass. No change needed.
THEORETICAL_LUTEIN_PER_BIOMASS = 0.00556 

# --- Global Variables for ODE solver ---
# Initial concentrations now set consistently in mg/L
C_x0_model, C_N0_model, F_in_model, C_N_in_model, I0_model = 0.5 * 1000, 1.0 * 1000, 8e-3, 10.0 * 1000, 150.0
...
def pbr(t, C):
    ...
    Concentrations C_X, C_N, C_L are now consistently in mg/L."""
    C_X, C_N, C_L = C
    if C_X < 1e-9: C_X = 1e-9
    if C_N < 1e-9: C_N = 1e-9
    if C_L < 1e-9: C_L = 1e-9

    # C_X is now in mg/L. Removed the * 1000 factor to be consistent with TAU's implied units.
    I = 2 * I0_model * (np.exp(-(TAU * 0.01 * C_X))) 
    Iscaling_u = I / (I + K_S + I ** 2 / K_I)
    Iscaling_k = I / (I + K_SL + I ** 2 / K_IL)
    u0 = U_M * Iscaling_u
    k0 = K_M * Iscaling_k

    # dCxdt, dCndt, dCldt will now produce rates in mg/L/hour
...
def calculate_biomass_cost(C_x0):
    """Calculate biomass cost based on initial concentration. C_x0 is in mg/L."""
    return (C_x0 / 1000) * VOLUME_L * PRICE_BIOMASS_PER_G # Convert mg to g for cost calculation

def calculate_nitrogen_cost(C_N0, C_N_in, F_in, time_hours):
    """Calculate nitrogen cost based on initial and feed concentrations. C_N0 and C_N_in are in mg/L."""
    initial_cost = (C_N0 / 1000) * VOLUME_L * PRICE_NITROGEN_PER_G # Convert mg to g
    feed_cost = (F_in * C_N_in / 1000) * time_hours * PRICE_NITROGEN_PER_G # Convert F_in rate unit (L/hour) * C_N_in (mg/L) to mg/hour, then to g/hour
...
def calculate_lutein_profit(lutein_concentration):
    """Calculate profit from lutein production. lutein_concentration is already in mg/L from ODE output."""
    # Removed the redundant * 1000. lutein_concentration (sol.y[2,-1]) is now directly in mg/L
    return lutein_concentration * VOLUME_L * PRICE_LUTEIN_PER_MG 
...
def calculate_lutein_yield(C_x_final, C_L_final):
    """Calculate the yield of lutein. Concentrations C_x_final, C_L_final are consistently in mg/L."""
    total_biomass_concentration = C_x_final # Assuming yield is based on total biomass concentration at end
    theoretical_lutein_concentration = THEORETICAL_LUTEIN_PER_BIOMASS * total_biomass_concentration
...
def _evaluate_lutein_model_objective(*args):
    ...
    final_lutein = sol.y[2, -1] # This value is now consistently in mg/L
    final_biomass = sol.y[0, -1] # This value is now consistently in mg/L

    if not np.isfinite(final_lutein) or final_lutein <= 0:
        return 1e6 # Return a very large number for bad results (minimization)
    
    if optimization_mode == "concentration":
        return -final_lutein # Minimize negative lutein to maximize lutein (now in mg/L)
    elif optimization_mode == "cost":
        cost_analysis = calculate_total_cost_and_profit(args, final_lutein, TIME_HOURS)
        return -cost_analysis['revenue_J'] # Minimize negative Revenue_J to maximize Revenue_J (gain)
...
# C_L_scaled is removed; C_L will directly be in mg/L
...
def set_optimization_mode():
    ...
    if optimization_mode == "concentration":
        ...
        p_conv.yaxis.axis_label = "Max Lutein Found (mg/L)" # Updated label
...
def get_current_dimensions():
    ...
    try:
        # Range values for C_x0, C_N0, C_N_in are now directly in mg/L
        return [
...
def reset_experiment():
    ...
    # These ranges are now in mg/L for the visualization context
    cx0, cn0 = cx0_range.value[1], cn0_range.value[1]
    cx_max, cn_max = cx0_range.end, cn0_range.end
    # The scaling factors (10000, 5000) below are based on the new mg/L max ranges of 10000 and 50000 respectively
    green_opacity = np.clip(cx0 / 10000.0, 0.0, 1.0) * 0.8 # Scale based on mg/L max
    blue_opacity = np.clip(cn0 / 10000.0, 0.0, 1.0) * 0.6 # Scale based on mg/L max
    ...
    def worker():
        try:
            ...
            for i, p in enumerate(points_to_calc):
                ...
                lutein_conc = sol.y[2, -1] # This value is now consistently in mg/L
                biomass_conc = sol.y[0, -1] # This value is now consistently in mg/L
                ...
                results.append({
                    'lutein': lutein_conc, # This is now in mg/L
            ...
            def callback():
                ...
                for i, res_idx in enumerate(nan_indices):
                    # Data passed to table should be mg/L, as the table headers are updated below
...
def suggest_next_experiment():
    ...
    def worker():
        ...
        try:
            ...
            metric_text = ""
            if optimization_mode == "concentration":
                predicted_lutein = -mean[0] # This is now in mg/L
                uncertainty = std[0]
                metric_text = f"<b>Predicted Lutein: {predicted_lutein :.4f} ± {uncertainty:.4f} mg/L</b>" # Updated label
            elif optimization_mode == "cost":
...
def run_suggestion():
    ...
    def worker():
        ...
        try:
            ...
            C_x0_model, C_N0_model, F_in_model, C_N_in_model, I0_model = temp_Cx0, temp_CN0, temp_Fin, temp_CNin, temp_I0
            lutein_val = sol.y[2, -1] # This is now in mg/L
            biomass_val = sol.y[0, -1] # This is now in mg/L
            ...
            def callback():
                ...
                new_point_data = {
                    ...
                    'I0': [point_to_run[4]],
                    'Lutein': [lutein_val], # This is in mg/L
                    'Total_Cost': [cost_analysis['total_cost']],
                    'Lutein_Profit': [cost_analysis['lutein_profit']],
                    'Revenue_J': [cost_analysis['revenue_J']],
                ...
                if optimization_mode == "concentration":
                    update_status(f"✅ Ran suggested experiment as Step {opt_step_number}. Lutein: {lutein_val:.4f} mg/L") # Updated label
                elif optimization_mode == "cost":
...
def process_and_plot_latest_results():
    ...
    max_lutein = sol.y[2, -1] # This value is now consistently in mg/L
    final_biomass = sol.y[0, -1] # This value is now consistently in mg/L
    final_nitrate = sol.y[1, -1] # This value is now consistently in mg/L
    ...
    if optimization_mode == "concentration":
        results_html += f"<b>Maximum Lutein Found:</b> {max_lutein:.4f} mg/L<br/>" # Updated label
    elif optimization_mode == "cost":
        ...
        results_html += f"<b>Lutein Concentration:</b> {max_lutein:.4f} mg/L<br/>" # Updated label
    ...
    # Adjusted scaling for opacity based on mg/L maximums
    # If 0.018 g/L was the maximum, that's 18 mg/L. Using a new max appropriate for mg/L scale, e.g., 50 mg/L for visual scaling
    orange_opacity = np.clip(max_lutein / 50.0, 0.0, 1.0) * 0.9 
    orange_gradient = f"linear-gradient(rgba(255, 140, 0, {orange_opacity}), rgba(210, 105, 30, {orange_opacity}))"
    # If 5.0 g/L biomass was max, that's 5000 mg/L. Using a new max appropriate for mg/L scale, e.g., 10000 mg/L
    green_opacity = np.clip(final_biomass / 10000.0, 0.0, 1.0) * 0.6 
    green_gradient = f"linear-gradient(rgba(48, 128, 64, {green_opacity}), rgba(24, 64, 32, {green_opacity}))"
    # If 2.0 g/L nitrate was max, that's 2000 mg/L. Using a new max appropriate for mg/L scale, e.g., 20000 mg/L
    blue_opacity = np.clip(final_nitrate / 20000.0, 0.0, 1.0) * 0.5 
    blue_gradient = f"linear-gradient(rgba(52, 152, 219, {blue_opacity}), rgba(41, 128, 185, {blue_opacity}))"
    liquid_css = f"""
        position: absolute; bottom: 0; left: 0; right: 0; height: 95%;
...
def update_convergence_plot_from_history():
    ...
    for _, y_val in opt_history:
        if optimization_mode == "concentration" or optimization_mode == "yield":
            value = -y_val # Convert back to actual concentration/yield (now in mg/L or %)
...
def run_final_simulation(best_params):
    ...
    initial_conditions = [best_params[0], best_params[1], 0.0] # Initial C_L is 0.0 mg/L
    sol = solve_ivp(pbr, [0, TIME_HOURS], initial_conditions, t_eval=t_eval, method="RK45")
    
    C_x0_model, C_N0_model, F_in_model, C_N_in_model, I0_model = temp_Cx0, temp_CN0, temp_Fin, temp_CNin, temp_I0
    simulation_source.data = {
        "time": sol.t, 
        "C_X": np.maximum(0, sol.y[0]), # Now in mg/L
        "C_N": np.maximum(0, sol.y[1]), # Now in mg/L
        "C_L": np.maximum(0, sol.y[2]), # Now in mg/L
        # Removed C_L_scaled: np.maximum(0, sol.y[2]) * 100 as it's no longer needed for mg/L plotting
    }
    return sol
...
# Updated ranges and titles to reflect mg/L scale
cx0_range = RangeSlider(title="C_x0 Range (mg/L)", start=0, end=10000, value=(200, 2000), step=10)
cn0_range = RangeSlider(title="C_N0 Range (mg/L)", start=0, end=10000, value=(200, 2000), step=10)
fin_range = RangeSlider(title="F_in Range (L/hr)", start=1e-5, end=1.5e-1, value=(1e-3, 1.5e-2), step=1e-4, format="0.0000") # F_in is a flow rate, units unchanged.
cnin_range = RangeSlider(title="C_N_in Range (mg/L)", start=0, end=50000, value=(5000, 15000), step=50)
...
# --- Data Table & Plots ---
columns = [
    TableColumn(field="C_x0", title="C_x0 (mg/L)", formatter=NumberFormatter(format="0.0000")), # Updated title
    TableColumn(field="C_N0", title="C_N0 (mg/L)", formatter=NumberFormatter(format="0.0000")), # Updated title
    TableColumn(field="F_in", title="F_in (L/hr)", formatter=NumberFormatter(format="0.0000")), # Updated title
    TableColumn(field="C_N_in", title="C_N_in (mg/L)", formatter=NumberFormatter(format="0.0000")), # Updated title
    TableColumn(field="I0", title="I0 (umol/m2-s)", formatter=NumberFormatter(format="0.0000")), # Updated title
    TableColumn(field="Lutein", title="Lutein (mg/L)", formatter=NumberFormatter(format="0.0000")), # Updated title
...
p_sim = figure(height=300, width=800, title="Simulation with Best Parameters", x_axis_label="Time (hours)", y_axis_label="Concentration (mg/L)", y_range=DataRange1d(start=0)) # Updated label
# Removed extra_y_ranges for C_L_scaled since all concentrations are now consistently mg/L
...
p_sim.line(x="time", y="C_L", source=simulation_source, color="orange", line_width=3, legend_label="Lutein (C_L)") # Used C_L directly, it's now mg/L
